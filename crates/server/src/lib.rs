...
pub struct AppState {
    ...
    pub gateway: Arc<gateway::ReliableClient>,
}

impl AppState {
    #[must_use]
    pub fn new() -> Self {
        let router = gateway::Router::new("claude-3-7-sonnet-20250219".to_string());
        let policy = gateway::RetryPolicy::default();
        let gateway_client = Arc::new(gateway::ReliableClient::new(router, policy));
...
pub fn app(state: AppState) -> Router {
    Router::new()
        ...
        .route("/gateway/chat", post(gateway_chat))
        .with_state(state)
}
...
async fn gateway_chat(
    State(state): State<AppState>,
    Json(payload): Json<api::MessageRequest>,
) -> ApiResult<Json<api::MessageResponse>> {
    match state.gateway.send_message(&payload).await {
        Ok(res) => Ok(Json(res)),
        Err(e) => Err((
